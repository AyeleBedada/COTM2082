...
function sameType(type,correct){
    if(type==="semester"){
        return [...new Set(courses.map(c=>c[4]))].filter(x=>x!==correct);
    }
    if(type==="code"){
        return courses.map(c=>c[1]).filter(x=>x!==correct);
    }
    if(type==="ects"){
        return [...new Set(courses.map(c=>String(c[2])))]
        .filter(x=>x!==correct);
    }
    if(type==="prerequisite"){
        return [...new Set(courses.map(c=>c[3]))]
        .filter(x=>x!==correct);
    }
}
...
function makeQuestion(){
    ...
    if(type==="semester"){
        return {
            type,c,
            q:`Which semester is "${c[0]}" offered in?`,
            a:c[4]
        };
    }
    ...
    if(type==="code"){
        return {
            type,c,
            q:`What is the course code of "${c[0]}"?`,
            a:c[1]
        };
    }
    ...
    if(type==="ects"){
        return {
            type,c,
            q:`How many ECTS does "${c[0]}" carry?`,
            a:String(c[2])
        };
    }
    ...
    return {
        type,c,
        q:`What is the prerequisite of "${c[0]}"?`,
        a:c[3]
    };
}
...
function nextQuestion(){
    ...
    // Clears explanation until the question is answered
    feedback.innerHTML="";
